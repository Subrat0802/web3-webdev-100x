...
const RestaurantCard = ({ data }) => {
  return (
    ...
    >
      ...
      />
      <div className="p-2">
        <p className="font-bold text-xl whitespace-nowrap overflow-hidden text-ellipsis">{data.name}</p>
        {/* <p>{data.costForTwo}</p> */}
        ...
        <div className="text-gray-600 dark:text-gray-400">{data ? <p>{data.areaName}</p> : <p>{data.locality}</p>}</div>
      </div>
    </div>
  );
};
